...
$distance = $_GET['km'];
$communes = $_GET['nom'];
if (!empty($communes) && !empty($distance)) {

    //Conversion de la variable distance en degré decimaux :
    $distance = $distance / 1000;
    //Preparation des requêttes pour recuperer les donnees dans la BD
    $reqCommunes = "SELECT ncc,ST_AsGeoJSON(communes.SHAPE) as coordonnees 
            FROM communes,centre_commercial 
            where ST_Intersects(communes.SHAPE,ST_Buffer(centre_commercial.SHAPE," . $distance . "))";

    $resultatreq = mysqli_query($connexion, $reqCommunes);
    //var_dump($resultatrequette);
    $communesData = array();
    while ($rows = mysqli_fetch_assoc($resultatreq)) {
        $communesData[] = array(
            "ncc" => utf8_encode($rows["ncc"]),
            "coordonnees" => $rows["coordonnees"]

        );
    }
    //echo json_encode($communesData);
                ...
                <div class="u-align-left u-container-style u-layout-cell u-size-43 u-layout-cell-1">
                    <div class="u-container-layout u-valign-bottom u-container-layout-1">
                        <div id="map" class="u-clearfix u-custom-html u-custom-html-1">
                            <script>
                                var Communes =<?php echo json_encode($communesData); ?>;
                        ...
                        <div class="formX u-form u-form-1" id="form1" style="display: none">
                            <form action="./display_data.php" method="get">
                                <div>
                                    ...
                                    <br>
                                    <input onchange="showKm()" type="range" id="km" name="km" min="0" max="20"
                                           style="vertical-align: text-bottom;"><span
                                            style="margin-right:25px; color: white" id="valueKM"></span>
                                </div>
                                <br>
